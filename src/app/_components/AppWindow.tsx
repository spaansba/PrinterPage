    ...
    <>
      <div className="w-[288px] border-t-2 border-l-2 border-white border-b-2 border-r-2 border-b-[#808080] border-r-[#808080] shadow-[2px_2px_8px_rgba(0,0,0,0.2)]">
        <TitleBar
          ...
          setPageActivated={setPageActivated}
        />
        {pageActivated === "Toast" && (
          <EditorPage
...
function useFriendList() {
  ...
  const deleteFriend = async (userId: string, friendToDelete: Friend) => {
    const result = await removeAssociatedPrinters(
      userId,
      friendToDelete.printerId,
    );
    if (!result.success) {
    ...
    setFriendList((prev) =>
      prev.filter((friend) => friend.printerId !== friendToDelete.printerId),
    );
  ...
  const changeFriendName = async (
    ...
    try {
      const changed = await changeNameAssociatedPrinters(
        userId,
        printerId,
        data.name,
      );
...
function getLastMessagedFriend(friendList: Friend[]): Friend[] | [] {
  ...
  const mostRecentFriend = friendList.reduce((mostRecent, current) => {
    ...
    return new Date(current.lastSendMessage) >
      new Date(mostRecent.lastSendMessage)
      ? current
      : mostRecent;
  });
  ...
  const tiedFriends = friendList.filter(
    (friend) =>
      friend.lastSendMessage &&
      new Date(friend.lastSendMessage).getTime() === mostRecentTime,
  );
