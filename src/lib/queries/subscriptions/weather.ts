    ...
    try {
      ...
      const weatherCards = await Promise.all(
        weather.forecast.map((forecast) =>
          drawWeatherCard(forecast, settings.Temperature),
        ),
      );
...
const getForecastInPeriods = (hourlyData: HourlyWeather[]) => {
  return PERIOD_CONFIGS.map((period) => {
    const periodData = period.hours
...
const averageBy = (arr: HourlyWeather[], key: keyof HourlyWeather) => {
  const sum = arr.reduce((acc, curr) => acc + (curr[key] as number), 0);
  return Number((sum / arr.length).toFixed(1));
};

const getMostFrequentCondition = (
  arr: WeatherCondition[],
): WeatherCondition => {
  ...
  const mostFrequentText = [...frequencyMap.entries()].sort(
    (a, b) => b[1] - a[1],
  )[0][0];
...
export const getWeatherReport = async (userLocation: string) => {
  ...
  try {
    ...
    const forecastInPeriods = getForecastInPeriods(
